...
output2 = {}
for index,(percent_hedged,weights_scenario) in enumerate(itertools.product(samples,weights)):
...
def shareholder_value():
    for i in range(0,iter):
        FCF1 = FCF1_std*np.random.randn() + FCF1_mean
        FCF2 = FCF2_std*np.random.randn() + FCF2_mean
        FCF3 = FCF3_std*np.random.randn() + FCF3_mean
        FCF4 = FCF4_std*np.random.randn() + FCF4_mean
        FCF5 = FCF5_std*np.random.randn() + FCF5_mean
        FCFT = FCFT_std*np.random.randn() + FCFT_mean
        g = g_std*np.random.randn() + g_mean
        wacc = wacc_std*np.random.randn() + wacc_mean
        debt = debt_std*np.random.randn() + debt_mean
        outcome = equity_calc(FCF1,FCF2,FCF3,FCF4,FCF5,FCFT,g,wacc,debt)
        df_res.iloc[i] = outcome
        outcome_mean = round(df_res.mean(),2)
        outcome_sd = round(df_res.std(),2)
    return outcome_mean, outcome_sd
