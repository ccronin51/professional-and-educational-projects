...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep  4 16:55:26 2018
...
import itertools
#%%
os.chdir('/Users/cronin/Desktop/Simulation/data/')
df1 = pd.read_csv('spot_prices_sample.csv')
df2 = pd.read_csv('futures_data_sample.csv')
#%%
weights = pd.read_csv('weights.csv',parse_dates=['train_end_date'])
weights = weights.set_index(['train_end_date'])
#%%
''' Metal Margin Plot '''
...
#%%
weights_range = pd.date_range(start=weights.index.min(), end = df1.index.max(), freq='D')
weights = weights.reindex(weights_range, method='ffill')
intercept = weights['intercept']
weights = weights.stack()
weights.name = 'multiplier'
#%%
df2 = df2.set_index('Commodity',append=True)
weights.index.names = ['Date','Commodity']
df2 = df2.join(weights.to_frame(),how='left')
#%%
...
sci['Commodity'] = 'SCI'
#%%
df1 = df1.rename(columns={'amm_busheling_midwest':'Busheling', 
                          'hr_coil':'HRC',  
                          'iron_ore':'SGX Iron Ore',
                          'turkish_scrap':'Turkish HMS'})


df1 = df1.stack()
df1.index.names = ['Date', 'Commodity']
df1 = df1.to_frame().join(weights.to_frame(),how='left')
df1 = df1.mul(df1['multiplier'],axis=0)
df1 = df1.drop(columns=['multiplier'])
df1 = df1.groupby(level=0).sum()
df1 = df1.add(intercept, axis=0)
#%%
# =============================================================================
...
for sample,weight in itertools.product(samples,weights):
    print(sample,weight)

for index,(sample,weight) in enumerate(itertools.product(samples,weights)):
    print(index,sample,weight)
...
for index,(percent_hedged_scenario,weights_scenario) in enumerate(itertools.product(samples,weights)):
    for x,(c_code,weight) in itertools.product(list(dates)[:-months],enumerate(weights_scenario)):
        ...
        except ZeroDivisionError: # FOR IF THERE's A WEIGHT OF ZERO
            continue
#%%
# ''' Long - Simulation '''
# output2 = {}
...
#%%
''' ADD - Metal Margin Function Attempt '''
# HRC(Revenue)
total_sold_per_month = 150000
weights =[.1]
